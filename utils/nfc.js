...
let writeCB = () => {}
// 要写入的数据
let text = 'https://m.toutiao.com/article/7335686572622348812/?from_page_type=feed&upstream_biz=toutiao_m&wid=1708087515036&W2atIF=1';
let readResult = '';
// 安卓配置结束
var cid = null;
export default {
	...
	listenNFCStatus: function() {
		...
		try {
			...
			console.log('intent::', plus.globalEvent)
	...
	android_write(intent) { //写代码
		...
		try {
			...
			// 这个是写入url
			let textRecord = new NdefRecord(NdefRecord.TNF_ABSOLUTE_URI, urls, plus.android.invoke('', "getBytes"),
				urls);
			// 这个是写入包名，识别芯片跳转app
	...
	writeData: function(data, cb) { // 更改写状态
		if (noNFC) {
	...
	makeReadOnly(intent) {
		...
		if (ndef.canMakeReadOnly()) {
			const reuslt = ndef.makeReadOnly()
			if (result) toast('设置只读成功，只读模式不可再写入任何信息')
			else toast('设置失败')
		} else {
			toast('此卡不支持该操作')
		}
